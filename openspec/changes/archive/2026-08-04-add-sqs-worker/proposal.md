...
- O worker faz polling da fila SQS configurada pela variável `ACHE_PRECO_BOM_SQS_QUEUE_URL` usando `SqsService.receiveMessage`
...
- **Variáveis de ambiente**: reutiliza `ACHE_PRECO_BOM_SQS_QUEUE_URL` já configurada por `MensagemService.receberMensagem`
