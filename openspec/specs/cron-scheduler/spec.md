...
O job de consumo da fila SQS do Padre Ramon DEVE permanecer em `src/cron/sqs-consumer/sqs-consumer.service.ts`. O job DEVE usar o decorador `@Cron()` ou registro dinamico via `SchedulerRegistry` com a expressao cron vinda da env `PADRE_RAMON_SQS_CRON` (padrao `0 0 8-18/2 * * *`). O job DEVE usar `SqsService` (de `src/aws/sqs/sqs.service.ts`) em vez de `AwsSqsService` para operacoes de SQS.

#### Scenario: Job registrado com expressao padrao
- **WHEN** a env `PADRE_RAMON_SQS_CRON` nao estiver definida
- **THEN** o cron DEVE usar `0 0 8-18/2 * * *`
...
- **THEN** o servico DEVE ler a fila SQS via `SqsService.receiveMessage()`
...
- **WHEN** a env `PADRE_RAMON_SQS_QUEUE_NAME` nao estiver configurada
